...
import type { Question, QuestionType } from "../service/manage-question-service";

type Props = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Question yang sedang diedit */
  question: Question | null;
  /** Simpan perubahan. Terima id + payload (tanpa id). */
...
export default function EditQuestionDialog({
  ...
  // ===== Prefill dari props.question saat open / question berubah
  useEffect(() => {
    if (!question) return;
    ...
    // mediaType opsional (kalau kamu belum pakai di payload, biarin aja sebagai UI hint)
    setMediaType(
      question.mediaUrl
        ? question.mediaUrl.match(/\.(mp3|wav)$/)
          ? "audio"
    ...
    } else if (question.type === "Fast Accuracy") {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* Batasi tinggi; padding aman di mobile; body pakai ScrollArea */}
          ...
          <ScrollArea className="flex-1">
            <form onSubmit={handleSubmit} className="px-4 sm:px-6 pb-6 pt-2 space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-1">
                <Button variant="outline" type="button" onClick={() => onOpenChange(false)} className="min-w-[96px]">
                  Cancel
                </Button>
